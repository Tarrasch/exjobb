...
\cite{stack_traces_ticket}. This chapter will be go into
...
that thunks loose the reference to the code that pushed the update frame (recall figure \ref{fig:reify_update_frame} and figure \ref{fig:thunks_and_blackholes}). So instead
...
say nothing about the over-applied function we have tail-called to.
...
The solutions will to the discussion in the previous paragraph have two natural solutions.
Either we find a way to reify the stack in full but conditionally,
compromising that stack reifications cost can be linear in time but only happen
...
The second approach would be to let the stack value be \emph{lazily evaluated}. Then we would only
...
and \ref{sec:chunked_reifying} are ideas of implementing lazy stack value. But first we take a look at a simple and
...
printed out, second, as we saw from subsection \ref{sec:update_frames},
...
One advantage of this idea is that we would never actually need to do
any reification in the sense that we create another array, so then all
payloads are still accessible. If the payloads are available from the
stack value in the handler, the optimizations from section \ref{sec:frames_of_interest}
could be configurable from code in Haskell-land.
...
referencing. The reified chunk could be stored in the current underflow
frame or inside the lazy stack value perhaps.
...
parents that they don't need to reify on underflows when no stack value
is alive anymore.
