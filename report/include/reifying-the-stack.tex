...
One advantage of this idea is that we would never actually need to do any
reification in the sense that we create another array, so then all payloads
are still accessible. Allowing for the stack trace optimizations from
section 5.1 TODO to be optional. A far fetched idea could even involve
printing out variables (the fields).

\subsection{Chunked reifying}

Lazy reification does not need the stack to become immutable. The stack
can only modify itself frame by frame unlike an array which can access
any element. This is a powerful property since this means that the stack
is almost immutable already. As the stack pointer moves down towoards
the bottom of the stack, we could start copying over frames before they
get overwritten. FORTSÄTTHÄR

For a fact, when we get an underflow, control is passed to the rts
