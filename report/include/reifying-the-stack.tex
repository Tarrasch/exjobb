...
A very simple solution would be to just reify 10 frames. 

\subsection{Static analysis}

By analyzing the code statically, we get an understanding which can
be put to use when deciding weather we should reify the stack or not.
We only would reify the stack if we're going to use the stack value.

\subsubsection{Dispatching call sites of throws}

If we know which functions are called from catch-wrapped code.

\subsubsection{Marking catch frames if they use the stack}

When we climb the stack looking for our catch-frame, it would be
great if the catch-frame knew if we need to reify or not.
