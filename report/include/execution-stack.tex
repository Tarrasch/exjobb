...
While there are publish papers on how GHC works, a lot of details have
since changed and the papers are now partially obsolete. Since even
small details of the execution stack will be important for this work,
all this information is based on the only definitive source of how the
...
% Tips: http://hackage.haskell.org/package/base-4.6.0.1/docs/Control-Concurrent.html#v:forkIO för nedanstående

Haskell's TODO library exports the 
\mint{haskell}|forkIO :: IO() -> IO()| primitive (source TODO).
Intuitively it just creates another "green" thread running its argument.
...
try to understand how many execution stacks are stored where they are
referenced from.
...
It should be noted that almost all functions in the run time library
...
They are all of closure type \texttt{RET_SMALL}
