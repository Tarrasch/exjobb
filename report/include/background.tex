...
% En ide är att skriva vad man tror att an läsare redan vet. Man kanske kan
% säga att vi antar nästan ingen tidigare Haskell erfarenhet.
...
We will not go into
details of how the Haskell language specification defines evaluating an
expression, but there is no concept of a stack or that functions return
values. Instead you think of it as successively applying equations to
eventually reach a completely evaluated value. For each time you apply
an equation, you remove one occurence of function on the equation's
left hand side (altough even more functions could be added). Figure
...
Haskell have no statements, only expressions. So obviously there
...
function calls and jumps in Haskell are implemented for GHC (the
compiler).
...
first step is turning \emph{one} occurence of \texttt{g} into \emph{two}
occurences by just applying equational reasoning. If running \texttt{g}
...
But should we really evaluate \texttt{(g 10)} if we don't have to? Most
Haskell implementations will only evaluate it once (if ever). Remember
that we can not in general reduce \texttt{(f (g 10))} to \texttt{(f
13)} (we can for this particular case, but not in general). That would
violate the lazyness property of the language. But how do we then
ensure that we only evaluate \texttt{(g 10)} once? The solution is to
substitute \texttt{(f (g 10))} with \texttt{(f t) where t = (g 10)}
where \texttt{t} is a \emph{Thunk}. Figure \ref{fig:thunks_er} shows the same value
being evalued as in figure \ref{fig:purity_er}, but with thunks. Note
that it required less steps to do evaluation with thunks and this always
holds in general.
