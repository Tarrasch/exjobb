...
Haskell and one in an the imperative language C.
...
occurences by applying the equation for \texttt{f}. If running \texttt{g}
...
But should we really evaluate \texttt{(g 10)} twice if we don't have to? Most
Haskell implementations will only evaluate it once (if ever). We can not
in general reduce \texttt{(f (g 10))} to \texttt{(f 13)}
(we can for this particular case, but not in general). That would
...
that it requires fewer steps to do evaluation with thunks.
