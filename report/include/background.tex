...
It's outside the scope of this paper to examine the IRs. In other words,
a compiler is successively recasting source code to lower and lower
abstractions until it becomes machine code.
...
Unfortunately, there is by automatic no way to associate the machine code to the
...
kilogramToGram = (*1000)
  \end{minted}
  \caption{Two functions with identical implementations. They will get
optimized into one, so a stack trace involving one of the two functions
can't exist.}
