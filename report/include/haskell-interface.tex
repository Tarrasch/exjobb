...
% TODO (peter said): With the notable exception that Haskell has no
% sub-typing, so "SomeExeception" becomes a wrapper for the concrete
% exception instead of a supertype. And yes, you explain this later.
...
% TODO(p said):  Strictly speaking you could make an (orphaned) exception
% instance for String
...
% TODO (peter):
% > toException, fromException etc.
%
% It's nice that you document all the tricks the exception system plays
% with the type system - but do we really need all this for stack traces?
% Is it exclusively so we can talk about SomeException alter?
...
% TODO Peter:
% > If the programmer catches an exception downcasted to say an
% >  ArithException
%
% ... it would contain no exception stack anymore. Which is problematic
% even if the handler itself doesn't care about it, because in the case
% of a re-throw we might still get to a handler that does.
%
% Probably debatable, but I think you're jumping to the conclusion a bit
% fast here. Especially given that (I think) you haven't mentioned the
% practice of re-throwing exceptions at all up to this point.


% TODO Peter:
%
% > This is particularly confusing for programmers used to Java
%
% As a rule, being confusing to Java programmers isn't really something
