...
say that we have a type called an \texttt{ExecutionStack}, values of
...
implementation from section \ref{sec:lazy_reifying} we use. This section
...
throwing at the same time. It is now time to look at possible solutions
that would give a decent Haskell-interface to the programmer.

\subsubsection{Solutions in pure Haskell}
...
But this would break backwards compatibility because you change the
type of \texttt{SomeException}. Furthermore it is not as convenient as
it seems. If the programmer catches an exception downcasted to say an
...
run the frame's handler with the exception passed as argument. The
...
have type signatures, this is a sanity check for GHC maintainers that
get an extra sanity check. The type signature for \texttt{catch\#} is:
\mint{haskell}|catch# :: IO a -> (exception -> IO a) -> IO a|
Note how the type system for primitives is weaker, we can not constrain
the type variable \texttt{exception} to be belong to the type class \texttt{Exception}.

There are many interesting possibilities here, for example in Peter
Wortmann's demonstration from \ref{sec:recent_work} he changed the
...
the stack value. Figure TODO illustrates when and
where a recovery frame would be pushed. Once the handler that is written
in Haskell gets control, we know that stack pointer \texttt{Sp} points
...
\begin{itemize}
  ...
  \item
    ...
    rethrowing we always want the next handler to get a new stack
    trace.
...
does not matter in Java. The way a language defines rethrowing can be summarized
in a table, for Java the table it would look like in table TODO. The
...
can not be like for Java. Instead we propose that Haskell have the
