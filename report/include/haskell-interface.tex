...
Contrast this to trying to deliberately giving a confusing API, say
that:
...
a constant value like \texttt{"<No data>"}. In the case of having a
execution stack frame that can be interpreted to correspond to multiple
...
capable of storing one info pointer per stack frame, so for this
decision it is also an implementation issue. However, one idea would be
...
Then that particular field could be heavily documented, mentioning the
rarity of its presence.
