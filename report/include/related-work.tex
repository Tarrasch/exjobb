...
In the end tough, none of the these methods are without it's drawbacks
naturally, otherwise we would not need stack traces in the first place.

As for statical analysis tools for Haskell, the Haskellwiki only
mentions a few examples. We'll look at one of them, Catch, which can
Detect common sources of runtime errors. \cite{haskellwiki_static_analysis_tools}
